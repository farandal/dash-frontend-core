...
// e.g., "dash.development", "dash.production", "pinoywok.ngrok"
const CUSTOM_MODE: string | undefined = process.env.CUSTOM_MODE;

// Determine config file name based on CUSTOM_MODE
// If CUSTOM_MODE is set (e.g., "dash.development"), use "config.dash.development.yaml"
...
const indexHtml = BUILD_ENV === "prod" ? path.join(app.getAppPath(),"apps/dash-app/dist/index.html"): path.join(appPath,"src/index.html");
...
const startPythonProcess = async (t: string, c: string) => {
  ...
  const outputFunction = (data: any) => {
    ...
    if (!connectionEstablished && (
      ...
      showNotification("DashAdmin", "✅ Background service connected successfully!");
