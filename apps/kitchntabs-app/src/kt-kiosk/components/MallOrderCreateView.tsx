...
import { MallCategorySelector } from './MallCategorySelector';
 ...
 * - Left sidebar with vertical scrollable store selector OR category selector
 * - Cart summary header
 * - Assistance button
...
export const MallOrderCreateView: React.FC<MallOrderCreateViewProps> = (props) => {
    ...
    const { categories } = useMallOrderCreate();
    ...
    // Determine if we should show categories or stores
    // If we have categories loaded (from categoriesPath), show category selector
    ...
    return (
        <Box className="kt-mall-client-tab-create-view">
            ...
            {/* Left Sidebar - Vertical Store/Category Selector */}
            <Box className="kt-mall-client-tab-sidebar">
                {showCategories ? <MallCategorySelector /> : <MallStoreSelector />}
            </Box>
