...
import { getCssVariableNumber } from '@dashadmin/dash-boilerplate';
...
// Inject CSS variables from tenant settings or defaults
const injectTenantStyles = () => {
    const tenantSettings = AuthPersistenceService.getTenantSettings();
    const currentMode = document.documentElement.getAttribute('data-theme') || DASHLayoutSettings.THEME_TYPE_LIGHT;
    
    try {
        if (tenantSettings) {
            const colors = tenantSettings.colors;
            console.log('Updating colors from local storage, mode:', currentMode);
            updateDomCssVariables(currentMode, colors, tenantSettings.values);
        } else {
            // No tenant settings (public/kiosk) - initialize defaults
            console.log('Initializing default colors, mode:', currentMode);
