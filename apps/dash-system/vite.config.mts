...
export default ({ mode }) => {
  ...
  // --- Workflow B: resolve workspace packages to their `src` for instant HMR (dev only) ---
  // dash-system develops the packages, so in dev it consumes their SOURCE directly:
  // edit packages/<pkg>/src/* → instant React Fast Refresh, no rebuild, no registry.
  // Production builds still resolve via each package's `exports` → `dist`.
  // Asset packages (dash-styles/.less, dash-modal/.mp3, dash-icons) stay on built dist.
  ...
  const devSrcAliases = isDevelopment
    ? SRC_HMR_PACKAGES.map((name) => ({
        // "pkg", "pkg/sub/path", and "pkg/src/sub/path" all → packages/pkg/src/sub/path
        // The (?:/src)? strips the explicit /src prefix so it isn't doubled.
