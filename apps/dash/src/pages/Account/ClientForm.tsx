...
import { IProfileForm } from './CustomProfile';
import { RutValidator } from '@app/utils/validators';


const ClientForm: FC<IProfileForm> = (props) => {
	...
	const formDefaultValues = {
		...
		'client.name': identity.client &&  identity.client.name ?  identity.client.name : '',
		avatar: null
	};
	...
	const updateProfile = async (values) => {
		...
		if (values.client && values.client.name) {
			formData.append('client[name]', values.client.name);
		}
						...
						<Grid item xs={12} md={4}>
							...
							>
								<SingleImageUploader
									...
									currentUrl={identity.image_url+identity.image_path}
									onChange={(file: File) => {
										setAvatar({
											rawFile: file,
						...
						<Grid item xs={12} md={4}>
							...
							<CardContent sx={{ p: 3 }}>
								...
								<div className='dash-app-form-item'>
									...
									{errors['client.name'] && (
										<span style={{ color: 'red' }}>
											{errors['client.name'].message as string}
										</span>
									)}
								</div>
