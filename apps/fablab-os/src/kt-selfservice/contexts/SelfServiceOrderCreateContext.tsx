...
import { dashStorage, priceFormatter } from 'dash-utils';
...
export const SelfServiceOrderCreateProvider: React.FC<PropsWithChildren> = ({ children }) => {
    ...
    // Format Price Helper - uses centralized priceFormatter
    const formatPrice = useCallback((amount: number | string) => {
        const val = typeof amount === 'string' ? parseFloat(amount) : amount;
        // Try to get currency from first product if available
        const currencyCode = allProducts?.[0]?.prices?.[0]?.pricelist?.currency?.code || 'CLP';
        return priceFormatter(isNaN(val) ? 0 : val, currencyCode);
    }, [allProducts]);
