...
export const formatPriceAdjustmentWithTenant = (price: any, currency?: any): string => {
    ...
    // Use centralized priceFormatter for the absolute value, then add +/- prefix
    const currencyCode = tenantCurrency?.code || 'CLP';
    const formatted = priceFormatter(Math.abs(numericPrice), currencyCode);
    // Remove the $ symbol from formatted since we want just +/- with the number
    const numericPart = formatted.replace(/^\$/, '');
    return numericPrice > 0 ? `+${numericPart}` : `-${numericPart}`;
};
