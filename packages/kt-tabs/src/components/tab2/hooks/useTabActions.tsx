...
export const useTabActions = (onTabClosed?: (tabId: number) => void) => {
    ...
    // State for payment methods and statuses
    ...
    useEffect(() => {
        const loadPaymentMethods = () => {
            ...
            try {
                ...
                const pointOfSales = getPointOfSales();
                
                if (pointOfSales && Array.isArray(pointOfSales)) {
                    const methods = pointOfSales
                        ...
                        .map(pos => { 
                            ...
                            active: pos.active,
                            is_default: pos.is_default || false,
                            connection_params: pos.connection_params || {}
                        })});
                    
                    if (methods.length === 0) {

                        setPaymentMethodsError(translate('tab.payment.no_active_methods'));
                        setPaymentMethods([]);
                    ...
                    setPaymentMethodsError(translate('tab.payment.no_methods_configured'));
                    setPaymentMethods([]);
                ...
                setPaymentMethodsError(translate('tab.payment.error_loading_methods'));
                setPaymentMethods([]);
    ...
    }, [auth, getPointOfSales, translate]);
