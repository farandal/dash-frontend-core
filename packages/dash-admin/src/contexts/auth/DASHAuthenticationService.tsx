...
class DASHAuthenticationService {
    ...
    // Method to handle redirect after successful login
    /*handleRedirectAfterLogin(backendRedirect?: string): string | null {
        return this.determineRedirectUrl(backendRedirect);
    }*/
...
async login(credentials: DASHAuthenticationServiceLoginCredentials): Promise<DASHAuthenticationServiceAuthResponse> {
    ...
    // New method to initialize auth from existing token
 /*   async initializeFromToken(): Promise<DASHAuthenticationServiceAuthResponse> {
        ...
        try {
            ...
            const redirectAfterLogin = this.handleRedirectAfterLogin(backendRedirect);
...
async initializeFromToken(): Promise<DASHAuthenticationServiceAuthResponse> {
    ...
    // Method to handle app initialization
    /*
            ...
            if (token && isAuthenticated && user) {
                ...
                if (shouldGetAuth) {
                    try {
                        ...
                        // Trigger auth context update with fresh data
                        setAuthEvent({
                            authenticated: true,
                            user: auth.user,
                            auth: auth.auth,
                        ...
                        const redirectAfterLogin = this.handleRedirectAfterLogin();
