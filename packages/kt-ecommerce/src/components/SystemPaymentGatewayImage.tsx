...
interface SystemPaymentGatewayImageProps extends IDashAutoAdminCustomFieldComponent {}

const SystemPaymentGatewayImageView: React.FC<SystemPaymentGatewayImageProps> = ({ 
  attribute 
}) => {
...
const SystemPaymentGatewayImageEdit: React.FC<SystemPaymentGatewayImageProps> = ({ 
  attribute 
}) => {
  const record = useRecordContext();
  ...
  const processFile = useCallback((file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert('Invalid file type. Use JPEG, PNG, GIF, WebP or SVG.');
      return false;
    }
  ...
  return (
    <Card sx={{ width: '100%' }}>
      <CardHeader title={attribute.label || "Logo"} />
      <CardContent>
        <Box
          component="label"
          ...
          {displayUrl ? (
            ...
            <Box display="flex" flexDirection="column" alignItems="center" gap={1}>
              ...
              <Typography variant="caption" color="textSecondary" textAlign="center">
                {isDragging ? 'Drop image here' : 'Drag an image or click'}
              </Typography>
            </Box>
          )}
          <input
...
const SystemPaymentGatewayImage = ({
  method,
  attribute,
  resourceConfig
  ...
  switch (method) {
    case 'edit':
    case 'create':
      return <SystemPaymentGatewayImageEdit attribute={attribute} method={method} resourceConfig={resourceConfig} />;
    case 'view':
      return <SystemPaymentGatewayImageView attribute={attribute} method={method} resourceConfig={resourceConfig} />;
  }
};

export default SystemPaymentGatewayImage;
