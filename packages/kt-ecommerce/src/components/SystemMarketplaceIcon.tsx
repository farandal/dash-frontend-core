...
const ZoomPortal: React.FC<ZoomPortalProps> = ({ src, alt, marketplaceName, mousePosition, isVisible }) => {
...
const SystemMarketplaceIconList: React.FC<IDashAutoAdminCustomFieldComponent> = ({ 
  method, 
  attribute, 
  resourceConfig 
}) => {
  ...
  const marketplaceName = record?.name;
    ...
    <>
      ...
      >
        <ImagePlaceHolder
          ...
          alt={marketplaceName || 'Marketplace icon'}
        />
      </div>
...
const SystemMarketplaceIcon = ({ 
  method, 
  attribute, 
  resourceConfig 
}: IDashAutoAdminCustomFieldComponent) => {
  return (
    <SystemMarketplaceIconList 
      attribute={attribute} 
      method={method} 
...
export default SystemMarketplaceIcon;
