...
const ZoomPortal: React.FC<ZoomPortalProps> = ({ src, alt, posName, mousePosition, isVisible }) => {
...
const SystemPointOfSaleIconList: React.FC<IDashAutoAdminCustomFieldComponent> = ({ 
  method, 
  attribute, 
  resourceConfig 
}) => {
    ...
    <>
      ...
      >
        <ImagePlaceHolder
          ...
          alt={posName || 'Point of Sale icon'}
        />
      </div>
...
const SystemPointOfSaleIcon = ({ 
  method, 
  attribute, 
  resourceConfig 
}: IDashAutoAdminCustomFieldComponent) => {
  return (
    <SystemPointOfSaleIconList 
      attribute={attribute} 
      method={method} 
...
export default SystemPointOfSaleIcon;
