...
const ComponentEdit: React.FC<IDashAutoAdminCustomFieldComponent> = ({ method, attribute }) => {
  ...
  return (
    <>
      <TextField label='Probar formateo' value={rawText} onChange={({ target: { value } }) => formatNumber(value)} />
      <TextField label='Valor formateado' value={formattedNumber} inputProps={{ readOnly: true, }} />
    </>
  )
}


const FormatCurrency = ({ method, attribute,resourceConfig}: IDashAutoAdminCustomFieldComponent) => {
  switch (method) {
    case "edit":
    case "create":
