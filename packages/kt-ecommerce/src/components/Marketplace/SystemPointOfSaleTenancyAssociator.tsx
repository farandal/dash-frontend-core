...
const ListAutoSelectIds = ({ resource, selectedIdsFn }) => {
    ...
    useEffect(() => {
        ...
        if (systemPointOfSale && systemPointOfSale?.tenancy_ids) {
            _selectedIds = [...new Set([..._selectedIds, ...systemPointOfSale.tenancy_ids])];
        }
...
export const SystemPointOfSaleTenancyAssociator: React.FC<IDashAutoAdminCustomFieldComponent> = ({
   resourceConfig }) => {
    ...
    useEffect(() => {
        ...
        setValue("tenancy_ids", systemPointOfSale?.systemPointOfSale ? systemPointOfSale.tenancy_ids.map(p => p.id) : []);
    ...
    return (<>
        {systemPointOfSale && (
            <>
                <h1>Tenancies asociadas al POS</h1>
                <Box sx={{ width: "100%" }}>
                    <List
                        ...
                        storeKey='system-point_of_sales-tenancies'
                        empty={<Loading />}
