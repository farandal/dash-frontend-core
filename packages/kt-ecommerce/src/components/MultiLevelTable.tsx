...
const inferStructure = (data) => {
  if (!data || !data.length) return null;

  const firstItem = data[0];
  const structure = {
    name: Object.keys(firstItem)[0],
    children: [],
    array: null
  };
  ...
  Object.keys(firstItem).forEach(key => {
    ...
    } else if (firstItem[key].length > 0) {
      // Handle nested array structure
      const arrayFirstItem = firstItem[key][0];
      structure.array = {
        name: key,
        children: Object.keys(arrayFirstItem).map(nestedKey => ({
          field: nestedKey,
          display: nestedKey.charAt(0).toUpperCase() + nestedKey.slice(1)
        }))
      };
    }
  });
...
const MultiLevelTable: React.FC<IMultiLevelTable> = ({
  ...
  if (!structure) {
    structure = inferStructure(data)
  }

  if (!structure) {
    return <>No structure for data provided</>
  }
